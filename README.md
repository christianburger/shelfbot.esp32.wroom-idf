...
The firmware pin map is maintained in [`pinout.md`](./pinout.md).

### LiDAR wiring (current firmware)

The current Lydsto/LD19-style setup in firmware is:

- `LiDAR TX -> ESP32 GPIO18` (UART RX)
- `LiDAR PWM -> ESP32 GPIO19` (PWM control output)
- `LiDAR GND -> ESP32 GND`
- `LiDAR +5V -> 5V rail`

See `sensor_control` initialization for authoritative runtime configuration.

### Feature table controls

Sensor drivers are explicitly gated in `sensor_control` by a feature table (`SENSOR_FEATURES`):

- `enable_ultrasonic`
- `enable_tof`
- `enable_lidar`

The web UI reads these settings via `/features` and conditionally shows ToF/LiDAR sections.
