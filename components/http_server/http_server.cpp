...
static esp_err_t tof_handler(httpd_req_t *req) {
    ...
    if (ToFSensorManager::instance().get_latest_readings(readings) && !readings.empty()) {
        ...
        cJSON_AddStringToObject(root, "error", "Failed to read ToF sensor");
...
static esp_err_t health_handler(httpd_req_t *req) {
    ...
    for (int i = 0; i < NUM_MOTORS; i++) {
        ...
        double pos = motor_control_get_position(i);
        bool motor_ok = (pos == pos); // Check for NaN

        cJSON_AddStringToObject(motor, "status", motor_ok ? "OK" : "ERROR");
        cJSON_AddNumberToObject(motor, "position", pos);
        cJSON_AddItemToArray(motors, motor);

        if (!motor_ok) all_motors_ok = false;
    }
    cJSON_AddItemToObject(root, "motors", motors);

    // Overall health
    bool system_healthy = tof_healthy && all_motors_ok;
    cJSON_AddStringToObject(root, "overall_status", system_healthy ? "HEALTHY" : "DEGRADED");
...
void start_webserver(void) {
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_uri_t tof_uri = { .uri = "/tof", .method = HTTP_GET, .handler = tof_handler, .user_ctx = NULL };
        httpd_register_uri_handler(server, &tof_uri);
