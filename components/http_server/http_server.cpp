...
esp_err_t HttpServer::register_uri_handlers(const httpd_handle_t server) {
    ...
    const httpd_uri_t routes[] = {
        ...
        { .uri = "/api/tof",         .method = HTTP_GET,  .handler = tof_handler,          .user_ctx = nullptr },
        { .uri = "/api/lidar",       .method = HTTP_GET,  .handler = lidar_handler,        .user_ctx = nullptr },
        { .uri = "/api/ultrasonic",  .method = HTTP_GET,  .handler = ultrasonic_handler,   .user_ctx = nullptr },
        { .uri = "/api/sensors",     .method = HTTP_GET,  .handler = sensors_handler,      .user_ctx = nullptr },
...
// ---------------------------------------------------------------------------
// Sensor status helper (shared — avoids copy-paste between Tof and Lidar)
// ---------------------------------------------------------------------------

std::string HttpServer::sensor_status_to_string(bool valid, int status) {
...
cJSON* HttpServer::create_sensor_json(const SensorCommon::SensorDataPacket& sensor_data) {
    ...
    cJSON_AddItemToObject(root, "ultrasonic", create_ultrasonic_json(sensor_data));
    cJSON_AddItemToObject(root, "tof",        create_tof_json(sensor_data));
...
esp_err_t HttpServer::motor_set_handler(httpd_req_t* req) {
    ...
    if (cJSON_IsNumber(motor)) {
        const uint8_t idx     = static_cast<uint8_t>(motor->valueint);
        const double pos_rad  = cJSON_IsNumber(pos) ? pos->valuedouble : 0.0;
        const double vel_rads = cJSON_IsNumber(vel) ? vel->valuedouble : 0.0;
...
esp_err_t HttpServer::health_handler(httpd_req_t* req) {
    ...
    if (has_sensor_data) {
        ...
        for (int i = 0; i < SensorCommon::NUM_ULTRASONIC_SENSORS; i++) {
            ...
            cJSON_AddBoolToObject  (sensor, "healthy",     sensor_data.ultrasonic_readings[i].valid);
        ...
        for (int i = 0; i < SensorCommon::NUM_TOF_SENSORS; i++) {
            cJSON* sensor = cJSON_CreateObject();
            cJSON_AddNumberToObject(sensor, "id",          i);
            cJSON_AddBoolToObject  (sensor, "healthy",     sensor_data.tof_measurements[i].valid);
