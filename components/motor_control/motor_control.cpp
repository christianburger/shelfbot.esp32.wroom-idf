...
static auto engine = FastAccelStepperEngine();
...
double motor_control_get_velocity(const uint8_t index) {
    ...
    double steps_per_sec = static_cast<double>(steppers[index]->getCurrentSpeedInMilliHz()) / 1000.0;
...
void motor_control_set_velocity(const uint8_t index, const double velocity_rad_s) {
    ...
    long speed_hz = static_cast<long>(fabs(velocity_rad_s * RADS_TO_STEPS));
...
void motor_control_stop_motor(const uint8_t index) {
    ...
    int dir_level = gpio_get_level(static_cast<gpio_num_t>(dir_pin));
...
void motor_control_set_motor_position_double(const uint8_t index, double position_deg) {
...
bool motor_control_move_all_motors_vector(const double* positions, const size_t num_positions, const long speed, bool non_blocking) {
