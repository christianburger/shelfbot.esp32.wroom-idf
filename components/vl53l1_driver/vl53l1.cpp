...
constexpr uint16_t VHV_CONFIG__TIMEOUT_MACROP    = 0x0008;
constexpr uint16_t PHASECAL_CONFIG__TIMEOUT_MACROP = 0x000B;
...
constexpr uint16_t SD_CONFIG__WOI_SD1            = 0x0079;
constexpr uint16_t SD_CONFIG__INITIAL_PHASE_SD0  = 0x007A;
constexpr uint16_t SD_CONFIG__INITIAL_PHASE_SD1  = 0x007B;
...
esp_err_t VL53L1_Driver::writeMulti(uint16_t reg, const uint8_t* src, size_t count) {
    ...
    return i2c_master_transmit(dev_handle_, tx.data(), tx.size(), timeout_ms_);
}
...
esp_err_t VL53L1_Driver::startContinuous() {
    // Back-to-back ranging mode
    return writeReg8(Reg::SYSTEM__MODE_START, 0x40);
}
...
const char* VL53L1_Driver::setup() {
    ...
    if (model_id != 0xEA) {
        ESP_LOGE(TAG, "Model ID mismatch: expected 0xEA, got 0x%02X", model_id);
        return "Wrong sensor model";
    }
    ESP_LOGI(TAG, "Model ID verified: 0x%02X", model_id);

    // Soft reset pulse
    err = writeReg8(Reg::SOFT_RESET, 0x00);
    if (err == ESP_OK) err = writeReg8(Reg::SOFT_RESET, 0x01);
    ...
    // Set I2C address register copy (7-bit address shifted)
    writeReg8(Reg::I2C_SLAVE_DEVICE_ADDRESS, static_cast<uint8_t>(i2c_address_ << 1));
...
const char* VL53L1_Driver::calibrate() {
    if (!initialized_) return "Not initialized";
    // Placeholder: keep lifecycle compatibility.
...
const char* VL53L1_Driver::check() {
    ...
    esp_err_t err = readReg8(Reg::IDENTIFICATION__MODEL_ID, &model_id);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Health check read failed: %s", esp_err_to_name(err));
        return "Health check failed";
    }
    if (model_id != 0xEA) {
        ESP_LOGE(TAG, "Health check model mismatch: 0x%02X", model_id);
        return "Health check failed";
    }
...
bool VL53L1_Driver::read_sensor(MeasurementResult& result) {
    ...
    if (err != ESP_OK) {
        did_timeout_ = (err == ESP_ERR_TIMEOUT);
    ...
    ESP_LOGI(TAG, "[READ] Measurement complete: %u mm valid=%d status=%u",
             result.distance_mm, result.valid ? 1 : 0, result.range_status);
