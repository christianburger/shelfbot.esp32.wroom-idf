...
constexpr int kIdReadRetries = 8;
} // namespace
...
esp_err_t VL53L1_Driver::readMulti(uint16_t reg, uint8_t* dst, size_t count) {
    ...
    // Use split write + read transactions (stop/start) for maximum compatibility
    // with VL53L1 modules/adapters that do not behave well with repeated-start.
...
const char* VL53L1_Driver::init() {
    ...
    // Let device boot after I2C bus initialization.
...
const char* VL53L1_Driver::setup() {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "FAILED to read model ID after %d attempts: %s", kIdReadRetries,
                 esp_err_to_name(err));
        ESP_LOGE(TAG,
                 "I2C address 0x%02X did not respond correctly. If this is a UART/Modbus VL53L1 module, use vl53l1_modbus driver instead.",
                 i2c_address_);
        return "Model ID read failed";
    }

    if (model_id != VL53L1_MODEL_ID) {
        ESP_LOGE(TAG, "Model ID mismatch: expected 0x%02X, got 0x%02X", VL53L1_MODEL_ID,
                 model_id);
...
const char* VL53L1_Driver::check() {
    ...
    if (err != ESP_OK || model_id != VL53L1_MODEL_ID) {
        ESP_LOGE(TAG, "Health check failed (err=%s model=0x%02X)", esp_err_to_name(err),
                 model_id);
