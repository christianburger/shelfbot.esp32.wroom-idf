...
static const char* TAG = "Lidar_Sensor";

LidarSensorArray::LidarSensorArray(uint8_t num_sensors)
    : num_sensors_(num_sensors) {
    sensors_.resize(num_sensors);
}

bool LidarSensorArray::add_sensor(uint8_t index, const LidarSensorConfig& cfg) {
    if (index >= num_sensors_) {
        ESP_LOGE(TAG, "Sensor index %d out of bounds", index);
        return false;
    }

    sensors_[index] = std::make_unique<LydstoLidar>(cfg.driver_config);
    const char* error = sensors_[index]->init();
    if (error != nullptr) {
        ESP_LOGE(TAG, "Failed to initialize LiDAR sensor %d: %s", index, error);
    ...
    if (error != nullptr) {
        ESP_LOGW(TAG, "Self-test warning for LiDAR sensor %d: %s", index, error);
    }

    return true;
}

bool LidarSensorArray::update_readings(std::vector<SensorCommon::Reading>& readings) {
...
LidarSensorManager::LidarSensorManager()
    : data_mutex_(nullptr)
...
bool LidarSensorManager::configure(const LidarSensorConfig* sensor_configs, uint8_t num_sensors) {
    if (array_) {
        ESP_LOGW(TAG, "Manager already configured");
        return false;
    }

    array_ = std::make_unique<LidarSensorArray>(num_sensors);

    bool any_sensor_configured = false;
    for (uint8_t i = 0; i < num_sensors; ++i) {
        if (array_->add_sensor(i, sensor_configs[i])) {
            any_sensor_configured = true;
            ESP_LOGI(TAG, "LiDAR sensor %d configured", i);
        }
    }
...
bool LidarSensorManager::start_reading_task(uint32_t read_interval_ms, UBaseType_t priority) {
    if (!array_) {
    ...
    if (xTaskCreate(reading_task, "lidar_reading", 4096, params, priority, &task_handle_) != pdPASS) {
        delete params;
        running_ = false;
        return false;
    }

    ESP_LOGI(TAG, "LiDAR reading task started with %" PRIu32 " ms interval", read_interval_ms);
    return true;
}

void LidarSensorManager::reading_task(void* param) {
    auto* params = static_cast<TaskParams*>(param);
    LidarSensorManager* self = params->manager;
...
bool LidarSensorManager::get_latest_readings(std::vector<SensorCommon::Reading>& readings) {
    if (!array_) {
        return false;
    }
...
void LidarSensorManager::resume() {
    paused_ = false;
}

void LidarSensorManager::stop() {
    running_ = false;
    if (task_handle_) {
