...
bool ToFSensor::read_single(ToFReading& reading, uint32_t timeout_ms) {
    if (!initialized_) {
        return false;
    }
    ...
    // Read distance
    uint16_t distance;
    if (!read_register_16(VL53L0X_Reg::RESULT_RANGE_STATUS + 10, distance)) {
        xSemaphoreGive(i2c_mutex_);
        return false;
    }
    ...
    reading.distance_mm = distance;
    reading.status = (range_status >> 3) & 0x0F;
    reading.signal_rate = 255; // Simplified - would need more reads
...
bool ToFSensorArray::read_all_single(ToFReading* readings, uint32_t timeout_ms) {
    if (!initialized_ || !readings) {
        return false;
    }
...
ToFSensorManager::~ToFSensorManager() {
    ...
    if (latest_readings_) delete[] latest_readings_;
...
bool ToFSensorManager::configure(const ToFArrayConfig& array_config,
    ...
    latest_readings_ = new ToFReading[num_sensors];
    reading_count_ = num_sensors;
...
void ToFSensorManager::reading_task(void* param) {
    ToFSensorManager* self = static_cast<ToFSensorManager*>(param);
    self->reading_task_impl();
}

void ToFSensorManager::reading_task_impl() {
    ToFReading* readings = new ToFReading[reading_count_];

    while (running_) {
        if (!paused_) {
            if (array_->read_all_single(readings, 100)) {
                // Update latest readings
                if (xSemaphoreTake(data_mutex_, pdMS_TO_TICKS(10)) == pdTRUE) {
                    memcpy(latest_readings_, readings, sizeof(ToFReading) * reading_count_);
                    xSemaphoreGive(data_mutex_);
                }

                // Call callback if registered
                if (callback_) {
                    callback_(readings, reading_count_);
                }
            }
        }
...
bool ToFSensorManager::get_latest_readings(ToFReading* readings, uint8_t max_count) {
    if (!readings || max_count < reading_count_) {
        return false;
    }

    if (xSemaphoreTake(data_mutex_, pdMS_TO_TICKS(100)) == pdTRUE) {
        memcpy(readings, latest_readings_, sizeof(ToFReading) * reading_count_);
        xSemaphoreGive(data_mutex_);
        return true;
    }
