...
struct UltrasonicSensorConfig {
    gpio_num_t trig_pin;
    gpio_num_t echo_pin;
    float collision_threshold_cm = 20.0f;
...
class UltrasonicSensorArray : public IUltrasonicArray {
public:
    ...
    bool readAll(std::vector<SensorCommon::Reading>& readings, uint32_t timeout_ms) override;
    size_t getSensorCount() const override { return num_sensors_; }
...
public:
    ...
    bool configure(const UltrasonicSensorConfig* sensor_configs, uint8_t num_sensors);
    bool start_reading_task(uint32_t read_interval_ms, UBaseType_t priority);
    bool get_latest_readings(std::vector<SensorCommon::Reading>& readings) const;
    void pause();
    void resume();
    void stop();
private:
    ...
    struct TaskParams { UltrasonicSensorManager* manager; uint32_t interval_ms; };
