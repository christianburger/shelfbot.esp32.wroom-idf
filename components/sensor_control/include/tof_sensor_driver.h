...
public:
    ...
    bool init();
    bool read_single(ToFReading& reading, uint32_t timeout_ms);
...
public:
    ...
    bool read_all_single(ToFReading* readings, uint32_t timeout_ms);
...
public:
    ...
    bool get_latest_readings(ToFReading* readings, uint8_t max_count);
...
private:
    ...
    void reading_task_impl();

private:
    // ORDER MATCHES CONSTRUCTOR INITIALIZER LIST
    ToFSensorArray*   array_;
    ToFReading*       latest_readings_;
    uint8_t           reading_count_;
    SemaphoreHandle_t data_mutex_;
    TaskHandle_t      task_handle_;
    bool              running_;
    bool              paused_;
    std::function<void(const ToFReading*, uint8_t)> callback_;
};
