...
#define CRED_COUNT (sizeof(s_creds) / sizeof(s_creds[0]))
...
static int scan_pick_best(void) {
    ...
    ESP_LOGI(TAG, "scan: %u APs found", count);

    int    best_cred = -1;
    int8_t best_rssi = INT8_MIN;

    for (int ci = 0; ci < CRED_COUNT; ci++) {
        if (s_creds[ci].ssid[0] == '\0') continue;
        for (uint16_t ai = 0; ai < count; ai++) {
            if (strcmp((char*)aps[ai].ssid, s_creds[ci].ssid) == 0) {
...
static esp_err_t start_connect(int ci) {
    ...
    ESP_LOGI(TAG, "connecting → \"%s\"", cfg.sta.ssid);
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (bits & _EVT_DISCONNECTED) {
                ESP_LOGW(TAG, "connection lost – scanning for new AP");
                s_connected_at_us = 0;
                break;
            }
